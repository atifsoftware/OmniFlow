/**
 * Enterprise Number-to-Words & Invoice Amount-in-Words Engine for OmniFlow
 * 
 * Supports:
 * - English (International: Million/Billion & South Asian: Lakh/Crore)
 * - Bangla (বাংলা: ০-৯৯, শত, হাজার, লক্ষ, কোটি)
 * - Currency Words for Invoices, Cheques, and Receipts (টাকা ও পয়সা মাত্র / Taka & Paisa Only)
 */

export const BANGLA_WORDS_0_TO_99: readonly string[] = [
...
export const ENGLISH_UNITS: readonly string[] = [
...
export interface EnglishNumberOptions {
  useLakhCrore?: boolean;
}
...
export class NumberToWords {
  static toEnglish(number: number | string, options: EnglishNumberOptions = {}): string {
    const num = Math.floor(Math.abs(parseFloat(String(number)) || 0));
  ...
  private static _englishBelowThousand(n: number): string {
    let str = '';
    if (n >= 100) {
  ...
  private static _englishLakhCrore(n: number): string {
    if (n === 0) return '';
    if (n >= 10000000) {
  ...
  private static _englishMillionBillion(n: number): string {
    if (n === 0) return '';
    if (n >= 1000000000) {
  ...
  static toBangla(number: number | string): string {
    const num = Math.floor(Math.abs(parseFloat(String(number)) || 0));
    if (num === 0) return 'শূন্য';
    return NumberToWords._banglaConvert(num).trim();
  }

  private static _banglaConvert(n: number): string {
    if (n === 0) return '';
    if (n >= 10000000) {
  ...
  static toCurrencyWords(amount: number | string, options: CurrencyWordsOptions = {}): string {
    const lang = (options.language || 'bn').toLowerCase() as 'bn' | 'en';
    ...
    const currencyConfig: Record<string, { en: { unit: string; subUnit: string }; bn: { unit: string; subUnit: string } }> = {
      BDT: {
